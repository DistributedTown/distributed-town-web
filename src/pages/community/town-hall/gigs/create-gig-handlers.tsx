...
import { asyncPoll, DitoLogoSvg, ParseSWErrorMessage, SwButton } from 'sw-web-shared';
import { ClaimMembershipErrorTypes, CreateGigErrorTypes } from '../../join/store/model';
import { createGig } from '../../store/gigs.reducer';
...
export const OnCreateGigHandlers = (
  ...
  const onIPFS = async (isCorrectNetwork: boolean, metadataJson: TextileBucketMetadata) => {
    if (!isCorrectNetwork) {
      return null;
    }
    ...
    try {
      ...
      setDialogContent(
        <DialogAdditionalActionNeeded
          ...
          handleAdditionalAction={() => handleAdditionalAction(CreateGigErrorTypes.RetryTextile)}
          onCancel={handleClose}
        />
      );
    }
  ...
  const onCreateGig = async (credits: string, ipfsUrl: string) => {
    if (!ipfsUrl) {
      return null;
    }
    try {
      setDialogContent(
        <DialogLoadingMessage subtitle="This might take awhile, please be patient" message="Posting your gig.." onCancel={handleClose} />
      );

      return await createGig({ credits, metadataUrl: ipfsUrl });
    } catch (error) {
      ...
      setDialogContent(
        <DialogAdditionalActionNeeded
          ...
          message={message}
          handleAdditionalAction={() => handleAdditionalAction(CreateGigErrorTypes.RetryCreate)}
          onCancel={handleClose}
        />
      );
    }
  ...
  //     const nonce = await generateNonce(NonceActions.CreateGig, tokenId);
  ...
  //           handleAdditionalAction={() => handleAdditionalAction(CreateGigErrorTypes.RetryNonce)}
  //           onCancel={handleClose}
  //         />
  //       );
  //     } else {
  ...
  //           handleAdditionalAction={() => handleAdditionalAction(CreateGigErrorTypes.RetryQrCode)}
  //           onCancel={handleClose}
  //         />
  //       );
  //     }
